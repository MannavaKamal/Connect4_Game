...
const Index = () => {
  ...
  const { mode, opponentConnected, timeRemaining, gameState,player2,currentPlayer,wsRef,gameId,position,col,moveTrigger,setCurrentPlayer,setGameState, startWaiting, sendMove, getBotMove } = useMultiplayer();

  const handleEnterGame = () => {
    console.log("iam kamal")
  ...
  const handleWin = async(winningPlayer: 1 | 2) => {
    setWinner(winningPlayer);
    if (winningPlayer === 1) {
      await axios.post(`${config.url}/insertgame`,{gameId,})
  ...
  // Auto-start game when opponent is connected
  useEffect(() => {
    if (gameState === 'waiting' && opponentConnected) {
      setTimeout(() => {
        console.log(mode)
        handleGameReady();
      }, 5000); // 1.5s delay to show connection message
    }
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-5xl">
          ...
          >
            <div className="bg-card rounded-3xl p-12 shadow-2xl text-center max-w-md">
              <h2 className="text-3xl font-bold mb-6 text-foreground">
                {JSON.parse(localStorage.getItem("userSession")).username}(you) vs {opponentConnected ? (
                                                                                        player2
                                                                                      ) : (
              ...
              {!opponentConnected ? (
                ...
                <div className="text-2xl font-bold text-green-500 mb-4 animate-bounce-in">
                  {mode === 'bot' ? '🤖 Bot Opponent Ready!' : '👤 Opponent Found!'}
                  <p className="text-sm text-muted-foreground mt-2">Starting game...</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        {gameState === 'playing' && (
          ...
          >
            <PlayerHUD
              player1Name={JSON.parse(localStorage.getItem("userSession")).username}
              player2Name={mode === 'bot' ? 'Bot' : player2}
